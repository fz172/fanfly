...
!(function () {
      ...
      if (E) {
        if (r) {
          var S,
            ...
            W = z[k],
            R = Rt(0, O[D], $[D]),
            V = y ? O[D] / 2 - B - R - q - C.mainAxis : F - R - q - C.mainAxis,
            Y = y ? -O[D] / 2 + B + R + W + C.mainAxis : I + R + W + C.mainAxis,
            U = e.elements.arrow && qt(e.elements.arrow),
            K = U ? ('y' === _ ? U.clientTop || 0 : U.clientLeft || 0) : 0,
            Q = null != (S = null == T ? void 0 : T[_]) ? S : 0,
            X = P + Y - Q,
            G = Rt(p ? jt(M, P + V - Q - K) : M, P, p ? St(N, X) : N);
          (E[_] = G), (L[_] = G - P);
        }
        if (a) {
          var nt,
        ...
        u = {
          ...
          forceUpdate: function () {
            if (!c) {
              ...
              if (Ae(e, n)) {
                (a.rects = {
                  reference: Ee(e, qt(n), 'fixed' === a.options.strategy),
                  popper: Nt(n)
                }),
                  (a.reset = !1),
  ...
  function nn() {
    this.complete &&
      (this.hasAttribute(Je) ? en.call(this, tn) : en.call(this, Ze));
  }
  function on() {
  ...
  var ln;
  const cn = document.getElementById('toc-bar'),
    un = document.getElementById('toc-solo-trigger'),
    fn = document.getElementsByClassName('toc-trigger'),
    dn = document.getElementById('toc-popup'),
    pn = document.getElementById('toc-popup-close'),
    hn = 'overflow-hidden',
    mn = 'closing';
  class gn {
    static initBar() {
      new IntersectionObserver(
        (t) => {
          t.forEach((t) => {
            cn.classList.toggle('invisible', t.isIntersecting);
          });
        },
        { rootMargin: `-${sn(gn, this, bn)._}px 0px 0px 0px` }
      ).observe(un),
        (vn._ = sn(gn, this, !1));
    }
    ...
    static refresh() {
      sn(gn, this, vn)._ && this.initComponents(),
    ...
    static showPopup() {
      this.lockScroll(!0), dn.showModal();
      dn.querySelector('li.is-active-li').scrollIntoView({ block: 'center' });
    }
    static hidePopup() {
      dn.toggleAttribute(mn),
        dn.addEventListener(
          'animationend',
          () => {
            dn.toggleAttribute(mn), dn.close();
          },
          { once: !0 }
        ),
        this.lockScroll(!1);
    ...
    static lockScroll(t) {
      document.documentElement.classList.toggle(hn, t),
        document.body.classList.toggle(hn, t);
    }
    static clickBackdrop(t) {
      if (dn.hasAttribute(mn)) return;
    ...
    static initComponents() {
      this.initBar(),
        ...
        (dn.onclick = (t) => this.clickBackdrop(t)),
        (pn.onclick = () => this.hidePopup()),
        (dn.oncancel = (t) => {
  ...
  an(gn, 'options', {
    ...
    scrollSmooth: !1,
    collapseDepth: 4,
    headingsOffset: sn((ln = gn), ln, bn)._
  });
  an(
    class {
      static refresh() {
  ...
  const yn = document.getElementById('mode-toggle');
  const _n = document.getElementById('sidebar'),
    wn = document.getElementById('sidebar-trigger'),
    En = document.getElementById('mask');
  class On {
    static toggle() {
      (xn._ = sn(On, this, !sn(On, this, xn)._)),
        document.body.toggleAttribute('sidebar-display', sn(On, this, xn)._),
        _n.classList.toggle('z-2', sn(On, this, xn)._),
        En.classList.toggle('d-none', !sn(On, this, xn)._);
    }
  }
  var xn = { _: !1 };
  const An = document.getElementById('sidebar-trigger'),
    Cn = document.getElementById('search-trigger'),
    Tn = document.getElementById('search-cancel'),
    Ln = document.querySelectorAll('#main-wrapper>.container>.row'),
    Sn = document.getElementById('topbar-title'),
    jn = document.getElementById('search'),
    kn = document.getElementById('search-result-wrapper'),
    Dn = document.getElementById('search-results'),
    Pn = document.getElementById('search-input'),
    Mn = document.getElementById('search-hints'),
    Nn = 'd-block',
    Bn = 'd-none',
    Fn = 'input-focus',
    In = 'd-flex';
  class Hn {
    ...
    static off() {
      Tn.classList.remove(Nn),
        jn.classList.remove(In),
        An.classList.remove(Bn),
        Sn.classList.remove(Bn),
        Cn.classList.remove(Bn);
    }
  }
  class $n {
    static on() {
      this.resultVisible ||
        (kn.classList.remove(Bn),
        Ln.forEach((t) => {
          t.classList.add(Bn);
        }),
    ...
    static off() {
      this.resultVisible &&
        ((Dn.innerHTML = ''),
        Mn.classList.contains(Bn) && Mn.classList.remove(Bn),
        kn.classList.add(Bn),
        Ln.forEach((t) => {
  ...
  function zn() {
    return Tn.classList.contains(Nn);
  }
  an($n, 'resultVisible', !1),
    (function () {
      const t = document.querySelectorAll('article img');
      if (0 === t.length) return;
      t.forEach((t) => {
        t.addEventListener('load', nn);
      }),
        document
          .querySelectorAll('article img[loading="lazy"]')
          .forEach((t) => {
            t.complete && en.call(t, Ze);
          });
    ...
    dayjs.locale(rn.locale),
    dayjs.extend(window.dayjs_plugin_localizedFormat),
    document.querySelectorAll(`[${rn.attrTimestamp}]`).forEach((t) => {
      const e = dayjs.unix(rn.getTimestamp(t)),
        n = e.format(rn.getDateFormat(t));
      if (
        ((t.textContent = n),
        t.removeAttribute(rn.attrTimestamp),
        t.removeAttribute(rn.attrDateFormat),
    ...
    (wn.onclick = En.onclick = () => On.toggle()),
    ...
    Pn.addEventListener('focusout', () => {
      jn.classList.remove(Fn);
    }),
    Pn.addEventListener('input', () => {
      '' === Pn.value
        ? zn()
          ? Mn.classList.remove(Bn)
          : $n.off()
        : ($n.on(), zn() && Mn.classList.add(Bn));
    }),
    yn &&
